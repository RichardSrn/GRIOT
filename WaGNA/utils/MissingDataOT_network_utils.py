...
def quantile(X, q, dim=None):
    ...
    dim : int or None, default = None
        Dimension along which to compute quantiles. If None, the tensor is flattened and one value is returned.
...
#### Accuracy Metrics ####
def MAE(X, X_true, mask) -> torch.Tensor:
    """
    ...
    if torch.is_tensor(X_true):
        X_true = X_true.clone().detach().cpu().numpy()

    if len(mask.shape) == 1:
        # if mask is 1 dimensional,
        # duplicate it so that is becomes 2-dimensional (n, m),
        # with n being the number of row of mask and m the number of columns of X
        mask = np.tile(mask, (X.shape[1], 1)).T

    mask_ = mask.astype(bool)
    mae = np.absolute(X[mask_] - X_true[mask_]).sum() / mask_.sum()

    # if 0 <= X_true.max() <= 1:
    #     assert mae <= 1 + 1e-6, f"MAE should be between 0 and 1, but got {mae:.2f}"

    return mae


def RMSE(X, X_true, mask) -> torch.Tensor:
    """
    ...
    mask_ = mask.astype(bool)
    rmse = np.sqrt(((X[mask_] - X_true[mask_]) ** 2).sum() / mask_.sum())

    # if X.max() <= 1 and X_true.max() <= 1:
    #     assert rmse <= 1 + 1e-6, f"RMSE should be between 0 and 1, but got {rmse:.2f}"
    return rmse
...
def MAR_mask(X, p, p_obs, seed=None):
    """
...
def MNAR_mask_logistic(X, adjacency_matrix, p, p_params=.3, exclude_inputs=True, seed=None):
    """
    ...
    adjacency_matrix : torch.DoubleTensor or np.ndarray, shape (n,n)
        Adjacency matrix to account for the network structure.
    ...
    idxs_params = rng.choice(d, d_params, replace=False) if exclude_inputs else np.arange(d)
...
def MNAR_self_mask_logistic(X, adjacency_matrix, p, seed=None, link_to_degree=True, device=torch.device("cpu")):
    """
    ...
    """
    n, d = X.shape
    # degree is the distribution of the nodes degree
    degree = adjacency_matrix.sum(axis=1)
    if link_to_degree:
        degree_coeff = (1 / ((degree + 1) ** 2))[:, None]
        degree_coeff = degree_coeff / degree_coeff.mean()
        degree_coeff = degree_coeff.to(device)
    else:
        degree_coeff = 1
    ...
    ### Pick coefficients so that W^Tx has unit variance (avoids shrinking)
    coeffs = pick_coeffs(X, self_mask=True, device=device)
    ### Pick the intercepts to have a desired amount of missing values
    intercepts = fit_intercepts(X, coeffs, p, self_mask=True, device=device)
    ps = torch.sigmoid(X * coeffs + intercepts)

    if seed:
        torch.manual_seed(seed)
    ber = torch.rand(n, d, device=device) if to_torch else np.random.rand(n, d)
    ber = ber / degree_coeff
    mask = ber < ps if to_torch else ber < ps.numpy()

    return mask


def MNAR_mask_quantiles(X, adjacency_matrix, p, q, p_params, cut='both', MCAR=False, seed=None):
    """
    ...
    idxs_na = rng.choice(d, d_na, replace=False)  ### select at least one variable with missing values
...
def pick_coeffs(X, idxs_obs=None, idxs_nas=None, self_mask=False, device=torch.device('cpu')):
    n, d = X.shape
    if self_mask:
        coeffs = torch.randn(d, device=device)
        Wx = X * coeffs
        coeffs /= torch.std(Wx, 0) + 1e-5
    else:
        d_obs = len(idxs_obs)
        d_na = len(idxs_nas)
        coeffs = torch.randn(d_obs, d_na, device=device)
        Wx = X[:, idxs_obs].mm(coeffs)
        coeffs /= torch.std(Wx, 0, keepdim=True)
    return coeffs


def fit_intercepts(X, coeffs, p, self_mask=False, device=torch.device("cpu")):
    if self_mask:
        d = len(coeffs)
        intercepts = torch.zeros(d, device=device)
        # for j in range(d):
        from tqdm import tqdm
        for j in tqdm(range(d), desc="MNAR", leave=False, position=11):
            def f(x):
                try:
                    ...
                    print("X*coeffs[j]+x:", X * coeffs[j] + x)
                    print("x:", x)
                    print("p:", p)
                    print(torch.sigmoid(coeffs[j]))
                    raise
