...
def load_planetoid_graph(path ="", name="CiteSeer", main_component_=True, directed=False, draw_=False):
    data_path = os.path.join(path, "data")

    os.makedirs(path, exist_ok=True)
    os.makedirs(data_path, exist_ok=True)

    # load citeseer dataset from torch_geometric.datasets
    dataset = Planetoid(root=data_path, name=name)
    data = dataset[0]
    ...
    if draw_:
        node_color = []
        nodelist = [[] for i in range(len(label_dict))]

        colorlist = ['#e41a1c', '#377eb8', '#4daf4a', '#984ea3', '#ff7f00', "#ffff33"]
        colorlist = np.array(colorlist)
        labels = F[:,0].to(int)  # data.y
