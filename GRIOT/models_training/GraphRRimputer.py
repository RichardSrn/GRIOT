...
class RRimputer():
    ...
    def fit_transform(self,
        ...
        imps = (self.noise * rand.to(self.device) + nanmean(F, 0))[mask.bool()]
        F[mask.bool()] = imps
        F_filled = F.clone()
        ...
        for i in tqdm(range(self.max_iters),
            ...
            for l in tqdm(range(d),
                ...
                for k in tqdm(range(self.n_iters),
                    ...
                    imps = self.models[j](
                        # F_filled_mask,
                        F_filled[:, np.r_[0:j, j + 1: d]],
                        # edge_index_mask
                        edge_index
                    ).squeeze()
                    F_filled[mask[:, j].bool(), j] = imps[mask[:, j].bool()]
        ...
        if F_true is not None:
            ...
            return F_filled, save_steps_path
    ...
    #
    #             with torch.no_grad():
    #                 F_filled[mask[:, j].bool(), j] = self.models[j](
    #                     F_filled[mask[:, j].bool(), :][:, np.r_[0:j, j + 1: d]]).squeeze()
    #
    #         if verbose and (i % report_interval == 0):
    #             if F_true is not None:
    #                 self.logging.info(f'Iteration {i}:\t '
    #                                   f'Validation MAE: {MAE(F_filled, F_true, data.test_mask.bool()).item():.4f}\t'
    #                                   f'RMSE: {RMSE(F_filled, F_true, data.test_mask.bool()).item():.4f}')
    #
    #         if torch.norm(F_filled - F_old, p=np.inf) < normalized_tol:
    #             break
    #
